        ...
        func swizzle() {
            ...
            swizzle(method) { previousImplementation -> Signature in
                return { session, request -> URLSessionDataTask in
                    ...
                    let request = delegate.feature?.urlSession(session, intercept: request) ?? request
                    ...
                    if #available(iOS 13.0, *) {
                        ...
                        delegate.feature?.urlSession(session, didCreateTask: task)
                    }
                    return task
                }
            }
